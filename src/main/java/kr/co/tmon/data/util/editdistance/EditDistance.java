...
import kr.co.tmon.data.util.editdistance.adapter.KoreanOptimizer;
import kr.co.tmon.data.util.editdistance.adapter.Optimizer;
		...
		for (int leftIndex = 1; leftIndex < distanceMatrix.length; leftIndex++) {
			for (int rightIndex = 1; rightIndex < distanceMatrix[leftIndex].length; rightIndex++) {
				if (left.charAt(leftIndex - 1) == right.charAt(rightIndex - 1)) {
					...
					distanceMatrix[leftIndex][rightIndex] = NumberUtils.min(
					        new int[] {
					                distanceMatrix[leftIndex - 1][rightIndex] + 1,
					                distanceMatrix[leftIndex - 1][rightIndex - 1] + 1,
					                distanceMatrix[leftIndex][rightIndex - 1] + 1
					        });
				}
			}
		}
